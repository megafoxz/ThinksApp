...
class Popup extends Component {
  ...
  start({ ...config }) {
    this.setState({
      ...
      buttonText: config.buttonText || "Cancel",
      buttonStyle: config.buttonStyle || "",
      button1: config.button1 !== undefined ? config.button1 : true,
      buttonText1: config.buttonText1 || "Ok",
      buttonStyle1: config.buttonStyle1 || "",
      callback:
        config.callback !== undefined
          ? config.callback
          : this.defaultCallback(),
      callback1:
        config.callback1 !== undefined
          ? config.callback1
          : this.defaultCallback(),
  ...
  render() {
    ...
    let el = null;
    if (this.state.button || this.state.button1) {
      el = (
        <View style={{ flexDirection: "row" }}>
          ...
          <TouchableOpacity
            style={[styles.Button1, styles[type]]}
            onPress={callback1}
          >
            <Text style={styles.TextButton1}>{buttonText1}</Text>
          </TouchableOpacity>
        </View>
      );
    } else {
